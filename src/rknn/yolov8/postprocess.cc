...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "/root/rknn_yolov8_demo/model/coco_80_labels_list.txt"
...
static int process_i8_combined(int8_t *input, int32_t zp, float scale,
                               int num_classes, int num_anchors,
                               std::vector<float> &boxes,
                               std::vector<float> &objProbs,
                               std::vector<int> &classId,
                               float threshold, int model_in_h, int model_in_w)
{
    int validCount = 0;
    int8_t score_thres_i8 = qnt_f32_to_affine(threshold, zp, scale);
    
    // Debug: print first few values
    printf("[DEBUG] First 10 quantized values: ");
    for (int i = 0; i < 10; i++) {
        printf("%d ", (int)input[i]);
    }
    printf("\n");
    printf("[DEBUG] score_thres_i8=%d\n", (int)score_thres_i8);
    
    // Sample some class scores from first anchor
    printf("[DEBUG] First anchor class scores (i8): ");
    for (int c = 0; c < 10; c++) {
        int8_t score = input[(4 + c) * num_anchors];
        float score_f = deqnt_affine_to_f32(score, zp, scale);
        printf("%d(%.3f) ", (int)score, score_f);
    }
    printf("\n");
    
    int stride_arr[3] = {8, 16, 32};
    int grid_sizes[3][2] = {{80, 80}, {40, 40}, {20, 20}};
    int anchor_offset = 0;
    
    for (int scale_idx = 0; scale_idx < 3; scale_idx++)
    {
        int grid_h = grid_sizes[scale_idx][0];
        int grid_w = grid_sizes[scale_idx][1];
        int stride = stride_arr[scale_idx];
        int grid_len = grid_h * grid_w;
        ...
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                int anchor_idx = anchor_offset + i * grid_w + j;
                ...
                {
                    int8_t score = input[(4 + c) * num_anchors + anchor_idx];
                ...
                if (max_score > score_thres_i8)
                {
                    // YOLOv8 outputs bbox in xywh format (center_x, center_y, width, height)
                    // relative to grid cell, need to convert to absolute coordinates
                    float box[4];
                    for (int k = 0; k < 4; k++)
                    {
                        box[k] = deqnt_affine_to_f32(input[k * num_anchors + anchor_idx], zp, scale);
                    }
                    
                    // Convert from grid-relative to image coordinates
                    // box[0], box[1] are center offsets from grid cell
                    // box[2], box[3] are width and height in grid units
                    float x_center = (j + box[0]) * stride;
                    float y_center = (i + box[1]) * stride;
                    float w = box[2] * stride;
                    float h = box[3] * stride;
                    
                    // Convert to x1, y1, w, h format for NMS
                    ...
                    objProbs.push_back(deqnt_affine_to_f32(max_score, zp, scale));
                    classId.push_back(max_class_id);
                    validCount++;
                }
            }
        }
        anchor_offset += grid_len;
    }
    
    printf("[DEBUG] dets before NMS: %d\n", validCount);
    return validCount;
}
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // Check if this is combined output format (YOLOv8 with single output [1, 84, 8400])
    if (app_ctx->io_num.n_output == 1)
    {
        printf("[INFO] Detected combined output format\n");
        int num_classes = app_ctx->output_attrs[0].dims[1] - 4;  // 84 - 4 = 80
        int num_anchors = app_ctx->output_attrs[0].dims[2];      // 8400
        printf("[DEBUG] num_classes=%d, num_anchors=%d\n", num_classes, num_anchors);
        printf("[DEBUG] zp=%d, scale=%f, threshold=%f\n", app_ctx->output_attrs[0].zp, app_ctx->output_attrs[0].scale, conf_threshold);
        
        if (app_ctx->is_quant)
        {
#if defined(RV1106_1103)
            validCount = process_i8_combined((int8_t *)_outputs[0]->virt_addr, 
                                            app_ctx->output_attrs[0].zp, 
                                            app_ctx->output_attrs[0].scale,
                                            num_classes, num_anchors,
                                            filterBoxes, objProbs, classId,
                                            conf_threshold, model_in_h, model_in_w);
#else
            validCount = process_i8_combined((int8_t *)_outputs[0].buf, 
                                            app_ctx->output_attrs[0].zp, 
                                            app_ctx->output_attrs[0].scale,
                                            num_classes, num_anchors,
                                            filterBoxes, objProbs, classId,
        ...
        {
            printf("[ERROR] FP32 combined output not implemented yet\n");
            return -1;
        }
    }
    else
    {
        // Original split output format processing
        printf("[INFO] Detected split output format\n");
#ifdef RKNPU1
